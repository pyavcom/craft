<?php
class Df_Core_Model_Resource_Config extends Mage_Core_Model_Mysql4_Config {
	/**
	 * @override
	...
	public function loadToXml(Mage_Core_Model_Config $xmlConfig, $condition = null) {
		 ...
		 * Так запрограммировано в методе @see Varien_Db_Adapter_Pdo_Mysql::prepareColumnValue():
				...
				case 'longtext':
					...
					break;
		 *
		 * Проблема решена методом @see Df_Core_Model_Resource_Config_Data::_prepareDataForSave()
		 */
	 ...
	 */
	public function updateByPath($path, $value, $valueOriginal = null) {
	 ...
	 * @param string $valueOriginal [optional]
	 */
	public function updateByPathLowLevel($path, $value, $valueOriginal = null) {
		rm_conn()->query(strtr(
			'UPDATE {table} SET value = \'{new value}\''
			.' WHERE (value = \'{old value}\') and path = \'{path}\';'
			,array(
				'{table}' => rm_table('core/config_data')
				,'{path}' => $path
				,'{old value}' => $valueOriginal
				,'{new value}' => $value
			)
		));
	}
