...
class Df_Directory_Model_Setup_Processor_Region extends Df_Core_Model_Abstract {
	...
	private function addRegionLocaleNameToDb(Mage_Directory_Model_Region $region, $localeCode) {
		...
		if ($isNew) {
			$this->getInstaller()->getConnection()
				->insert(
					$this->getTableCountryRegionName()
					,$dto
				)
			;
		}
	...
	/** @return string */
	private function getTableCountryRegionName() {
	...
	protected function _construct() {
		parent::_construct();
		$this
			->_prop(self::$P__REGION, Df_Directory_Model_Setup_Entity_Region::_CLASS)
			->_prop(self::$P__INSTALLER, Df_Core_Model_Resource_Setup::_CLASS)
		;
	}
	...
	/**
	 * @static
	 * @param Df_Directory_Model_Setup_Entity_Region $region
	 * @param Df_Core_Model_Resource_Setup $installer
	 * @return Df_Directory_Model_Setup_Processor_Region
	 */
	public static function i(
		Df_Directory_Model_Setup_Entity_Region $region, Df_Core_Model_Resource_Setup $installer
	) {
