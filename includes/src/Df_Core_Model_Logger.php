...
class Df_Core_Model_Logger extends Df_Core_Model_Abstract {
	...
	/** @return void */
	private function prepareFileDir() {
		/** @var array(string => bool) $prepared */
		static $prepared = array();
		if (!isset($prepared[$this->getFileDir()])) {
			if (!is_dir($this->getFileDir())) {
				mkdir($this->getFileDir(), 0777, $recursive = true);
			}
			else {
				chmod($this->getFileDir(), 0777);
			}
			$prepared[$this->getFileDir()] = true;
		}
	}
