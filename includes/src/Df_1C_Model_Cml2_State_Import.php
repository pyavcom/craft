<?php
class Df_1C_Model_Cml2_State_Import extends Df_Core_Model_Abstract {
	/** @return Df_1C_Model_Cml2_State_Import_Collections */
	public function collections() {return Df_1C_Model_Cml2_State_Import_Collections::s();}
				...
				$this->{__METHOD__} =
					Df_1C_Model_Cml2_File::i(
						Df_1C_Model_Cml2_Session_ByIp::s()->getFilePathById(
							Df_1C_Model_Cml2_Import_Data_Document_Catalog::TYPE__PRODUCTS
		...
		if (!isset($this->{__METHOD__})) {
			if (
				...
				$this->{__METHOD__} =
					Df_1C_Model_Cml2_File::i(
						Df_1C_Model_Cml2_Session_ByIp::s()->getFilePathById(
							Df_1C_Model_Cml2_Import_Data_Document_Catalog::TYPE__STRUCTURE
	...
	public function getFileCurrent() {
		if (!isset($this->{__METHOD__})) {
			...
			$this->{__METHOD__} = Df_1C_Model_Cml2_File::i($relativePath);
		}
		return $this->{__METHOD__};
	}
		...
		if (!isset($this->{__METHOD__})) {
			if ($this->getDocumentCurrentAsOffers()->isBase()) {
				...
				$this->{__METHOD__} =
					Df_1C_Model_Cml2_File::i(
						Df_1C_Model_Cml2_Session_ByIp::s()->getFilePathById(
							Df_1C_Model_Cml2_Import_Data_Document_Offers::TYPE__BASE
		...
		if (!isset($this->{__METHOD__})) {
			if ($this->getDocumentCurrentAsOffers()->hasPrices()) {
				...
				$this->{__METHOD__} =
					Df_1C_Model_Cml2_File::i(
						Df_1C_Model_Cml2_Session_ByIp::s()->getFilePathById(
							Df_1C_Model_Cml2_Import_Data_Document_Offers::TYPE__PRICES
							, $this->getDocumentCurrentAsOffers()->getExternalId()
						)
					)
				;
		...
		if (!isset($this->{__METHOD__})) {
			if ($this->getDocumentCurrentAsOffers()->hasStock()) {
				...
				$this->{__METHOD__} =
					Df_1C_Model_Cml2_File::i(
						Df_1C_Model_Cml2_Session_ByIp::s()->getFilePathById(
							Df_1C_Model_Cml2_Import_Data_Document_Offers::TYPE__STOCK
							, $this->getDocumentCurrentAsOffers()->getExternalId()
						)
					)
				;
	...
	private function getRootCategoryId() {
		if (!isset($this->{__METHOD__})) {
			/** @var int $result */
			$result = rm_nat0(rm_state()->getStoreProcessed()->getRootCategoryId());
			if (0 === $result) {
