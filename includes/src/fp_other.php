...
function df_exception_get_trace(Exception $exception, $showCodeContext = false) {
	return Df_Qa_Model_Message_Failure_Exception::i(array(
		...
		,Df_Qa_Model_Message_Failure_Exception::P__SHOW_CODE_CONTEXT => $showCodeContext
	))->getTraceAsText();
}
...
function df_notify($message) {
	if ($message instanceof Exception) {
		...
		Df_Qa_Model_Message_Notification::i(
			array(
				Df_Qa_Model_Message_Notification::P__NOTIFICATION => rm_sprintf($arguments)
...
function df_notify_admin($message, $doLog = true) {
	...
	Df_Qa_Model_Message_Notification::i(array(
		...
		,Df_Qa_Model_Message_Notification::P__NEED_LOG_TO_FILE => $doLog
		,Df_Qa_Model_Message_Notification::P__FILE_NAME => 'rm.admin.log'
		,Df_Qa_Model_Message_Notification::P__NEED_NOTIFY_ADMIN => true
 ...
 * Если у вас нет объекта класса Exception, то используйте df_notify
 *
 * @param Exception|string $exception
 * @param string|null $filename[optional]
 * @return void
 */
function df_notify_exception($exception, $filename = null) {
...
function df_notify_me($message, $doLog = true) {
	...
	Df_Qa_Model_Message_Notification::i(array(
		Df_Qa_Model_Message_Notification::P__NOTIFICATION => $message
		,Df_Qa_Model_Message_Notification::P__NEED_LOG_TO_FILE => $doLog
		,Df_Qa_Model_Message_Notification::P__FILE_NAME => 'rm.developer.log'
		,Df_Qa_Model_Message_Notification::P__NEED_NOTIFY_DEVELOPER => true
	))->log();
}
 ...
 * @return void
 */
function rm_adapt_legacy_object(Varien_Object $object) {
 ...
 */
function rm_caller($stackLevel = 1) {
	/** @var array(string => string|int) $bt */
	$bt = debug_backtrace();
	// Обратите внимание, что по умолчанию объект Df_Qa_Model_Debug_Execution_State
	// конструируется на основе $stackLevel = 2, то есть «родитель родителя»:
	// объект содержит данные о том методе, который вызвал тот метод, который вызвал метод rm_caller.
	return Df_Qa_Model_Debug_Execution_State::i(df_a($bt, $stackLevel + 1));
}
...
function rm_file_put_contents($filePath, $contents) {
	...
	df()->file()->createDirectory(dirname($filePath));
	/** @var int|bool $r */
