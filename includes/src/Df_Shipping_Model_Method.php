...
class Df_Shipping_Model_Method extends Mage_Shipping_Model_Rate_Result_Method {
	 ...
	 * @return string
	 */
	private function formatWeight($weightInKilogrammes) {
		return rm_sprintf(is_int($weightInKilogrammes) ? '%d' : '%.1f', $weightInKilogrammes);
	}
		...
		$this->throwException(
			strtr(
				...
				,array(
					...
					,'{предел веса}' => $this->formatWeight($maxWeightInKilogrammes)
					,'{вес заказа}' => $this->getRequest()->getWeightInKilogrammes()
				)
			)
		);
	}
