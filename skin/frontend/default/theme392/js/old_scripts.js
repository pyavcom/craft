...
jQuery(document).ready(function(){
		jQuery(function(){
	    jQuery('.form-language, .header .block-currency, .toolbar .sort-by, .toolbar .limiter, #product-options-wrapper .input-box , .block-order-return .return-select, ').jqTransform({imgPath:'<?php echo $this->getSkinUrl("images/") ?>'});
		});
		jQuery(function()
{
});
...
/***********************************************/
		...
		jQuery(".block-cart-header").hover(function() {
			...
			jQuery(".block-cart-header .cart-content").stop(true, true).delay(300).slideDown(500, "easeOutBounce");
		}, function() {
			jQuery(".block-cart-header .cart-content").stop(true, true).delay(300).fadeOut(800, "easeInCubic");
		});; 
...
jQuery(document).ready(function() {
  ...
  if(jQuery(".col-wrapper .slider").length){jQuery('.col-left').addClass('sidebar-indent-1'); };
  if(jQuery(".col-wrapper .banner-top").length){jQuery('.col-left').addClass('sidebar-indent-2'); };
   if(jQuery(".block-content .scroll-pane").length){jQuery('.block-content').addClass('block-content-extra'); };
   jQuery('.list-icon li').each(function(i){
  i++;
    jQuery(this).addClass('item-'+i);
   }); 
   jQuery('.list-icon li').hover(function(){
	...
	jQuery('.footer ul li a').hover(function(){
	   ...
	   jQuery(this).stop(true,false).animate({color:"#989898"}, {duration: 300});
	 });
