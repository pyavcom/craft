...
abstract class Df_InTime_Model_Method extends Df_Shipping_Model_Method_Ukraine {
		...
		return
			implode(
				...
				,array(
					...
					,rm_sprintf('%s', $this->formatTimeOfDelivery($this->getDeliveryTimeInDays()))
				)
			)
		;
	}
	...
	private function getDeliveryTimeInDays() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} = Df_InTime_Model_Request_DeliveryTime::i(array(
				Df_InTime_Model_Request_DeliveryTime::P__LOCATION_NAME_ORIGIN =>
					$this->getRequest()->getOriginCity()
				,Df_InTime_Model_Request_DeliveryTime::P__LOCATION_NAME_DESTINATION =>
					$this->getRequest()->getDestinationCity()
				,Df_InTime_Model_Request_DeliveryTime::P__NEED_GET_CARGO_FROM_THE_SHOP_STORE =>
					$this->getRmConfig()->service()->needGetCargoFromTheShopStore()
				,Df_InTime_Model_Request_DeliveryTime::P__NEED_DELIVER_CARGO_TO_THE_BUYER_HOME =>
					$this->needDeliverToHome()
			)
			)->getResultInDays();
		}
	...
	private function getFactorVolume() {
		...
		$result =
			...
			?
				...
				250
			:
					rm_float(df_a($this->getRates(), 'm3'))
		;
		return $result;
	}			
