...
class Df_YandexMarket_Helper_Data extends Mage_Core_Helper_Data {
	...
	public function preprocessUrl($url) {
		...
		/** @var string $scheme */
		$scheme = df_array_first($schemeAndOther);
		/** @var string $schemeTail */
		/**
		 * Раньше тут стояло:
		 * $schemeTail = df_array_last($schemeAndOther).
		 * Причина смены кода объяснена в предыдущем комментарии выше.
		 */
		$schemeTail = implode('//', df_array_tail($schemeAndOther));
		...
		$domainAndPort = df_array_first($schemeTailExploded);
		if ($this->settings()->other()->useNonStandardDomain()) {
			$domainAndPort = $this->settings()->other()->getDomain();
		}
		/** @var string[] $path */
		$pathExploded= df_array_tail($schemeTailExploded);
