<?php
abstract class Df_1C_Model_Cml2_Import_Data_Entity_AttributeValue
	...
	protected function createMagentoAttribute() {
		...
		$result = df()->registry()->attributes()->findByCodeOrCreate(
			$this->getAttributeCodeNew(), $this->getCreationParams()
		);
	...
	private function getCreationParams() {
		if (!isset($this->{__METHOD__})) {
			 ...
			 * @see Df_1C_Model_Cml2_Import_Data_Entity_AttributeValue_Barcode::getAttributeTemplate()
			 * @var Df_1C_Model_Cml2_Import_Data_Entity_Attribute_Date $template
			 */
