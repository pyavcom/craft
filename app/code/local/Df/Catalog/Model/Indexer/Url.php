...
class Df_Catalog_Model_Indexer_Url extends Mage_Catalog_Model_Indexer_Url {
	...
	{
		if (!df_enabled(Df_Core_Feature::SEO) || !$event->getDataObject()->getExcludeUrlRewrite()) {
			//
			...
			// в процессе массовой заметы адресных ключей товарных разделов.
			//
			// А избегаем мы перестройки по той причине,// что что за массовой заменой адресных ключей товарных разделов
			// мы хотим сначала выполнить такую же массовую замену адресных ключей товаров,// и лишь затем перестроить перенаправления.
			//
