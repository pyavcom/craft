<?php
class Df_Payment_Model_Config_Facade extends Df_Payment_Model_Config_Abstract {
	...
	/** @return Df_Payment_Model_Config_Area_No */
	public function noArea() {
		return $this->getConfig(Df_Payment_Model_Config_Area_No::_CLASS);
	}
	...
	public function getVar($key, $defaultValue = null) {
		...
		/** @var Df_Payment_Model_Config_Abstract $processor */
	...
	private function getConcreteConfigForKey($standardKey) {
		...
		/** @var Df_Payment_Model_Config_Abstract $result */
		$result = $this->noArea();
		...
		if (!isset($this->{__METHOD__}[$configClass])) {
			...
			$result =
				df_model(
					$configClass
					,array(
						Df_Payment_Model_Config_Abstract::P__CONST_MANAGER =>
							$this->getConstManager()
						,Df_Payment_Model_Config_Abstract::P__VAR_MANAGER =>
							$this->getVarManager()
					)
				)
			;
			df_assert($result instanceof Df_Payment_Model_Config_Abstract);
	...
	const P__CONFIG_CLASS__SERVICE = 'config_class__payment_service';
}