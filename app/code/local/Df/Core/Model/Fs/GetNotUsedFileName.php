...
class Df_Core_Model_Fs_GetNotUsedFileName extends Df_Core_Model_Abstract {
	...
	private function getVariables() {
		if (!isset($this->{__METHOD__})) {
			...
			$result =
				array_merge(
					array(
						'{date}' => df_dts($time, 'y-MM-dd')
						,'{time}' => df_dts($time, 'HH-mm')
						,'{time-full}' => df_dts($time, 'HH-mm-ss')
					)
	 ...
	 * @return Df_Core_Model_Fs_GetNotUsedFileName
	 */
	public static function i($directory, $nameTemplate, array $variables = array()) {return new self(array(
