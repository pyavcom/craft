...
function df_assert_class($value, $class, $stackLevel = 0) {
	if (df_enable_assertions()) {
		_df_qa_method()->validateValueClass($value, $class, $stackLevel + 1);
	}
}
...
function df_assert_string_not_empty($value, $stackLevel = 0) {
	...
	if (df_enable_assertions()) {
		 ...
		 */
		if ('' === strval($value)) {
			_df_qa_method()->raiseErrorVariable(
...
function df_param_between($resultValue, $paramOrdering, $min = null, $max = null, $stackLevel = 0) {
	if (df_enable_assertions()) {
		_df_qa_method()->assertParamIsBetween(
...
function df_result_between($resultValue, $min = null, $max = null, $stackLevel = 0) {
	if (df_enable_assertions()) {
		_df_qa_method()->assertResultIsBetween($resultValue, $min, $max, $stackLevel + 1);
	}
}
 ...
 * @throws Exception
 */
function df_should_not_be_here($method) {
	df_param_string($method, 0);
	_df_qa_method()->raiseErrorShouldNotBeHere($method);
}
...
function df_warning($message = null) {
	if ($message instanceof Exception) {
		$message = Df_Core_Exception::getMessageStatic($message);
	}
	else {
		if (is_array($message)) {
			$message = implode("\r\n\r\n", $message);
...
function rm_exception_to_session(Exception $exception) {
	/** @var string $message */
	$message =
		df_text()->nl2br(Df_Core_Model_Output_Xml::s()->outputHtml(
			Df_Core_Exception::getMessageStatic($exception)
		))
	;
	if (
...
function rm_int($value, $allowNull = true) {
	...
	if (is_array($value)) {
		...
		foreach ($value as $valueItem) {
			/** @var mixed $valueItem */
			$result[]= rm_int($valueItem, $allowNull);
		}
	}
	else {
		if (is_int($value)) {
