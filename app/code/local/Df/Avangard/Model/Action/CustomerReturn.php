<?php
class Df_Avangard_Model_Action_CustomerReturn extends Df_Payment_Model_Action_Confirm {
	/**
	 * @override
	 * @return Df_Avangard_Model_Action_CustomerReturn
	 */
		...
		try {
			if (!$this->getOrder()->canInvoice()) {
				...
				if ($this->getResponseState()->isPaymentServiceError()) {
					...
					$responseRegistration->loadFromPaymentInfo($this->getOrderPayment());
				 ...
				 * потому что Df_Avangard_Model_Action_CustomerReturn:process()
	...
	private function getRequestState() {
		if (!isset($this->{__METHOD__})) {
			$this->{__METHOD__} =
				Df_Avangard_Model_Request_State::i(array(
					Df_Avangard_Model_Request_State::P__ORDER_PAYMENT =>$this->getOrder()->getPayment()
					,Df_Avangard_Model_Request_State::P__PAYMENT_METHOD =>$this->getPaymentMethod()
				))
			;
		}
		return $this->{__METHOD__};
	...
	/**
	 * @static
	 * @param Df_Avangard_CustomerReturnController $controller
	 * @return Df_Avangard_Model_Action_CustomerReturn
	 */
	public static function i(Df_Avangard_CustomerReturnController $controller) {
