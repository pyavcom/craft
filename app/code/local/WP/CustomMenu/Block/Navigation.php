...
class WP_CustomMenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawCustomMenuItem($category, $level = 0, $last = false)
    {
        ...
        if ($drawPopup)
        {
            ...
            $html[] = '<div id="popup' . $id . '" class="wp-custom-menu-popup" onmouseout="wpHideMenuPopup(this, event, \'popup' . $id . '\', \'menu' . $id . '\')">';
            // --- draw Sub Categories ---
...
$_imgUrl = $_category->getImageUrl();
    $_imgHtml   = '';
        $_imgHtml = '<img class="img-menu" src="'.$_imgUrl.'" alt="'.$this->htmlEscape($_category->getName()).'" title="" width="180" height="195" />';
        $_imgHtml = $_helper->categoryAttribute($_category, $_imgHtml, 'image');


                $html.= '<a class="itemMenuName level' . $level . $active . '" href="' . $this->getCategoryUrl($child) . '"><span>' . $name . '</span><span>' . $_imgHtml . '</span></a>';
